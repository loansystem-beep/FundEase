<!-- resources/views/borrowers/groups/create.blade.php -->

@extends('layouts.app')

@section('content')
<div class="container">
    <h1>Create Borrower Group</h1>
    <p>If you give group loans, you can use this page to create a group of borrowers.</p>

    <form method="POST" action="{{ route('borrower-groups.store') }}">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('borrower_ids') is-invalid @enderror" id="borrower_ids" name="borrower_ids[]" multiple required>
                @foreach($borrowers as $borrower)
                    <option value="{{ $borrower->id }}" @if(in_array($borrower->id, old('borrower_ids', []))) selected @endif>{{ $borrower->full_name }} - {{ $borrower->id }}</option>
                @endforeach
            </select>
            @error('borrower_ids')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('group_leader_id') is-invalid @enderror" id="group_leader_id" name="group_leader_id">
                ...
                @foreach($borrowers as $borrower)
                    <option value="{{ $borrower->id }}" @if(old('group_leader_id') == $borrower->id) selected @endif>{{ $borrower->full_name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('loan_officer_id') is-invalid @enderror" id="loan_officer_id" name="loan_officer_id">
                ...
                @foreach($loanOfficers as $officer)
                    <option value="{{ $officer->id }}" @if(old('loan_officer_id') == $officer->id) selected @endif>{{ $officer->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('collector_name') is-invalid @enderror" id="collector_name" name="collector_name" value="{{ old('collector_name') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('meeting_schedule') is-invalid @enderror" id="meeting_schedule" name="meeting_schedule" value="{{ old('meeting_schedule') }}">
        ...
        <button type="submit" class="btn btn-primary">Create Borrower Group</button>
    </form>
</div>
@endsection
