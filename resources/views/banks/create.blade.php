...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Page Heading -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Add New Bank Account</h1>
        </div>

        <!-- Form -->
        <form action="{{ route('banks.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow border border-gray-200 space-y-6">
            @csrf

            <!-- Bank Code -->
            <div>
                <label for="code" class="block text-sm font-medium text-gray-700">Bank Code</label>
                <input type="text" name="code" id="code" placeholder="E.g. 001, ACC-INV, etc."
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('code') }}" required>
                @error('code') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                <div class="mt-2 text-sm text-gray-500">
                    Unique identifier for internal reference or journal grouping.
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="bank_account_name" id="bank_account_name" placeholder="E.g. ABC Bank Main Account"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('bank_account_name') }}" required>
                @error('bank_account_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                <div class="mt-2 text-sm text-gray-500">
                    This name appears in accounting and payment sections. It’s usually the name of the bank account (e.g. "Equity Main").
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="account_name" id="account_name" placeholder="E.g. ABC Ltd, John Doe, etc."
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('account_name') }}" required>
                @error('account_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                <div class="mt-2 text-sm text-gray-500">
                    This is the registered name the bank account is under.
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-2 text-sm text-gray-500">
                    Used for all financial transactions and journal entries linked to this account.
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-2 text-sm text-gray-500">
                    Limits which branches can access or transact using this bank account.
                </div>
            </div>
